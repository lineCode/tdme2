...
void LightingShaderBaseImplementation::useProgram(Engine* engine)
{
	...
	if (renderer->isInstancedRenderingAvailable() == true) {
		if (uniformProjectionMatrix != -1) renderer->setProgramUniformFloatMatrix4x4(uniformProjectionMatrix, renderer->getProjectionMatrix().getArray());
		if (uniformCameraPosition != -1) renderer->setProgramUniformFloatVec3(uniformCameraPosition, renderer->getCameraPosition().getArray());
		if (uniformCameraMatrix != -1) renderer->setProgramUniformFloatMatrix4x4(uniformCameraMatrix, renderer->getCameraMatrix().getArray());
	}
